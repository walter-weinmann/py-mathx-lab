...
def main(argv: list[str] | None = None) -> int:
    ...
    args = parse_experiment_args(
        experiment_id="e070",
        description="Primes in residue classes: pi(x; q, a).",
        argv=argv,
    )

    run_log = infer_run_log_file(out_dir=args.out_dir, experiment_slug="e070")
    setup_logging(config=LoggingConfig(verbose=args.verbose, log_file=run_log.log_file))
    logger.info("Starting experiment E070: Primes in residue classes: pi(x; q, a).")
