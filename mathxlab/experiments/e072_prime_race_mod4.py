...
def main(argv: list[str] | None = None) -> int:
    ...
    args = parse_experiment_args(
        experiment_id="e072",
        description="Prime race mod 4: pi(x;4,3) vs pi(x;4,1).",
        argv=argv,
    )

    run_log = infer_run_log_file(out_dir=args.out_dir, experiment_slug="e072")
    setup_logging(config=LoggingConfig(verbose=args.verbose, log_file=run_log.log_file))
    logger.info("Starting experiment E072: Prime race mod 4: pi(x;4,3) vs pi(x;4,1).")
