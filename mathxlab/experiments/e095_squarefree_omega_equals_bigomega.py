...
from mathxlab.experiments.number_theory_suite import run_e095
...
def main() -> int:
    ...
    args = parse_experiment_args(
        experiment_id="e095",
        description="Squarefree filter: ω(n)=Ω(n) when μ(n)≠0",
    )

    run_log = infer_run_log_file(out_dir=args.out_dir, experiment_slug="e095")
    ...
    logger.info("Starting experiment E095")
    run_e095(
    ...
    logger.info(
        "Experiment E095 completed successfully. Artifacts saved to: %s",
        args.out_dir,
    )
    return 0
