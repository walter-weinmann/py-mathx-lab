...
from mathxlab.experiments.prime_suite import run_e125
...
def main() -> int:
    ...
    args = parse_experiment_args_with_size(
        experiment_id="e125",
        description="E125: Sacks spiral structure.",
        size_default=301,
        size_help="Grid size parameter (odd). The experiment visualizes integers 1..size^2.",
    )

    run_log = infer_run_log_file(out_dir=args.out_dir, experiment_slug="e125")
    ...
    logger.info("Starting experiment E125")
    run_e125(
    ...
    logger.info("Experiment E125 completed successfully. Artifacts saved to: %s", args.out_dir)
    return 0
