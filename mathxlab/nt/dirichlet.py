...
class _TwoPowerComponent(_Component):
    ...
    For k >= 3, (Z/2^kZ)^× ≅ C2 × C_{2^{k-2}} and can be generated by -1 and 5.
    """
    ...
    def build(*, k: int) -> "_TwoPowerComponent":
        ...
        if k == 1:
            ...
            return _TwoPowerComponent(modulus=m, params=((0,),), k=k, order_cyc=1, gen_cyc=1, log_cyc={1: 0})
        if k == 2:
            ...
            return _TwoPowerComponent(modulus=m, params=params, k=k, order_cyc=order, gen_cyc=gen, log_cyc=log_map)
        ...
        for a_sign in (0, 1):
            for b in range(order_cyc):
                params.append((a_sign, b))
        return _TwoPowerComponent(modulus=m, params=tuple(params), k=k, order_cyc=order_cyc, gen_cyc=gen_cyc, log_cyc=log_cyc)
...
def conductor(chi: DirichletCharacter) -> int:
    ...
    for f in divs:
        ...
        for a in range(q):
            ...
            va = chi(a)
            for b in range(q):
                if a % f == b % f and gcd(b, q) == 1:
                    if abs(va - chi(b)) > 1e-12:
                        ok = False
                        break
            if not ok:
                break
...
def orthogonality_matrix(q: int) -> np.ndarray:
    ...
    return (vals @ np.conjugate(vals).T) / float(phi_q)
