...
If you have not seen Taylor polynomials before, think of them as a *polynomial that imitates a smooth function near one point*.

- Pick a center point $x_0$.
- Match the function value and the first few derivatives at $x_0$.
- This produces a polynomial $T_n(x; x_0)$ of degree $n$ that usually works best when $x$ is close to $x_0$.

For $\sin(x)$ around $x_0=0$ the first terms are:

$$
\sin(x) \approx x - \frac{x^3}{3!} + \frac{x^5}{5!} - \cdots
$$

In this experiment we do *not* start with a proof. We **plot the error**
$E_{n,x_0}(x)=|\sin(x)-T_n(x;x_0)|$ to see where the approximation is good and where it fails.

For a short beginner-level refresher, see {doc}`../background/taylor-series`.
...
- **Reproducibility:** run with a fixed `--seed` and persist parameters and figures under `--out`.
## How to run
...
The run writes artifacts to `out/e001/` (figures, parameters, and a short `report.md`).
For the documentation, summarize at least one concrete observation from the artifacts
and keep a small record that can be compared across runs.

| Degree set (`degrees`) | Center set (`centers`) | Domain      | Key observation (replace with your run)                                                             |
|------------------------|------------------------|-------------|-----------------------------------------------------------------------------------------------------|
| `{1, 3, 5, 7, 9}`      | `{0, π/2, π}`          | `[-2π, 2π]` | TODO: e.g. “low-error region expands around x0 with increasing n; far from x0 error grows quickly.” |

Typical patterns you should expect (and explicitly verify in the plots) are: the approximation improves rapidly near
the expansion center, the “good” region widens with degree, and improvements are not uniform over a fixed interval.
On wide domains or at higher degrees, you may also see behavior dominated by floating-point effects rather than pure
truncation error; this is a feature of the experiment, not a bug, and it motivates choosing domains/degree ranges
that separate numerical artifacts from the mathematical signal.
## Notes / pitfalls
