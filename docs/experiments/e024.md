...
```
*Prime counts shown are $\pi(\mathrm{size}^2)$, i.e., the number of primes $\le \mathrm{size}^2$.*
...
- **Finite procedure:** build a finite spiral window and run a deterministic primality test up to $\mathrm{size}^2$.
...
- **Computation:** map integers $1,\dots,\mathrm{size}^2$ to lattice coordinates on a square spiral; mark primes.
...
- **Prime classification:** primes are computed using a sieve up to $\mathrm{size}^2$.
...
- Larger `size` means more numbers ($\mathrm{size}^2$) and a longer run time.
...
The Ulam spiral is computed on a `size × size` grid, so runtime and memory scale roughly with $\mathrm{size}^2$ (because about $\mathrm{size}^2$ integers are classified as prime/not-prime).
