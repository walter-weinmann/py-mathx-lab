...
```
*Prime counts shown are π(size^2), i.e. the number of primes ≤ size^2.*
...
The Ulam spiral is computed on a `size × size` grid, so runtime and memory scale roughly with `size^2` (because about `size^2` integers are classified as prime/not-prime).
