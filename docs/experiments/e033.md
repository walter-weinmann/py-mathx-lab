...
```{figure} ../_static/experiments/e033_hero.png
:width: 80%
:alt: Preview figure for E033
```
...
- Recommended: `make run EXP=e033`
- Direct: `uv run --extra dev python -m mathxlab.experiments.e033`

Optional arguments:

- `make run EXP=e033 ARGS="--seed 123 --n 200000"`
...
- {doc}`e025` (Prime gaps are not monotone)
- {doc}`e026` (Normalized prime gaps)
- {doc}`e028` (Jumping champions (most frequent gaps))
- {doc}`e034` (Twin primes in sliding windows)
